...
from nerfstudio.fields.feature_field import FeatureNerfactoField
...
@dataclass
class FeatureNerfactoModelConfig(ModelConfig):
    """Nerfacto Model Config"""

    _target: Type = field(default_factory=lambda: FeatureNerFacto)
    ...
    train_regular_field: bool = True


class FeatureNerfactoModel(Model):
    """Nerfacto model
    ...
    config: FeatureNerfactoModelConfig
    ...
    def populate_modules(self):
        ...
        # Fields
        self.field = FeatureNerfactoField(
            self.scene_box.aabb,
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        # get all parameters from field except for field.dense_feature_head
        field_params = list(self.field.named_parameters())
        field_params = [p[1] for p in field_params if "dense_feature_head" not in p[0]]

        # get dense feature head parameters
        dense_feature_head_params = list(self.field.named_parameters())
        dense_feature_head_params = [
            p[1] for p in dense_feature_head_params if "dense_feature_head" in p[0]
        ]

        if self.config.train_regular_field:
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        feature = self.renderer_rgb(
            rgb=field_outputs[FieldHeadNames.FEATURE], weights=weights
        )
