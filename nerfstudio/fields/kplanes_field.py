...
class KPlanesField(Field):
    ...
    def __init__(
        ...
        if self.linear_decoder:
            ...
            #     },
            # )
            self.sigma_net_1 = nn.Linear(self.feature_dim, 64)
            self.sigma_net_act = nn.ReLU()
            self.sigma_net_2 = nn.Linear(64, self.geo_feat_dim + 1)
            ...
            self.dense_feature_head = tcnn.Network(
    ...
    def get_ndf_features(
        self, positions: TensorType, times: Optional[TensorType] = None
        ...
        if self.linear_decoder:
            raise ValueError("Linear decoder not supported for NDF features")
        else:
            ndf_features = self.sigma_net_1(grid_features).view(
                *positions.shape[:-1], -1
            )
            features = self.sigma_net_act(ndf_features)
            features = self.sigma_net_2(features)
    ...
    def get_outputs(
        ...
        density_embedding, ndf_features = (
            density_embedding[..., : self.geo_feat_dim],
            density_embedding[..., self.geo_feat_dim :],
        )
        ...
        dense_features = self.dense_feature_head(
            density_embedding.view(-1, self.geo_feat_dim)
        ).view(*output_shape, -1)

        return {
            FieldHeadNames.RGB: rgb,
            FieldHeadNames.FEATURE: dense_features,
        }
