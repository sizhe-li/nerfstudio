...
class SE3WarpingField(nn.Module):
    ...
    def get_transform(self, positions, warp_code, windows_param=None, covs=None):
        ...
        feat = torch.cat([encoded_xyz, warp_code], dim=-1).reshape(-1, encoded_xyz.shape[-1] + warp_code.shape[-1])
        feat = self.mlp_stem(feat)  # (R*S, D)
...
class HyperSlicingField(nn.Module):
    ...
    def forward(self, positions, warp_code=None, window_param=None, covs=None):
        ...
        feat = torch.cat([p, warp_code], dim=-1).reshape(-1, p.shape[-1] + warp_code.shape[-1])
        feat = self.mlp(feat)
...
class HyperNeRFField(Field):
    ...
    def __init__(
        ...
        self.density_net = tcnn.Network(
            n_input_dims=self.mlp_base.get_out_dim(),
            n_output_dims=1,
            network_config={
                "activation": "ReLU",
                "output_activation": "None",
        ...
        self.color_net = tcnn.Network(
                n_input_dims=self.mlp_head.get_out_dim(),
                n_output_dims=3,
                network_config={
                    "otype": "FullyFusedMLP",
                    "activation": "ReLU",
    ...
    def get_density(
        ...
        if self.use_integrated_encoding:
            ...
            if self.spatial_distortion is not None:
                ...
                gaussian_samples = (gaussian_samples + 2.0) / 4.0
            ...
            if warp_field is not None:
                assert "warp" in code_dict or "slice" in code_dict
                slice_code = code_dict["slice"] if "slice" in code_dict else code_dict["warp"]
        ...
        N, S = base_inputs.shape[:2]

        base_inputs = base_inputs.reshape(-1, base_inputs.shape[-1])
        base_mlp_out = self.mlp_base(base_inputs)

        density = self.density_net(base_mlp_out)
        density = trunc_exp(density - 1)

        return density.reshape(N, S, -1), base_mlp_out.reshape(N, S, -1), warped_directions
    ...
    def get_outputs(
        ...
        head_inputs = torch.cat(head_inputs, dim=-1)
        N, S = head_inputs.shape[:2]
        head_inputs = head_inputs.reshape(-1, head_inputs.shape[-1])

        mlp_out = self.mlp_head(head_inputs)
        return {
            FieldHeadNames.RGB: self.color_net(mlp_out).reshape(N, S, -1)  
        }
    ...
    def forward(
        ...
        for k in embeddings:
            if k == "camera":
                ...
                sample_inds = ray_samples.sample_inds.squeeze(2)  # [R, S]
                code_dict[k] = embeddings[k](sample_inds)  # [R, S, D]
    ...
    def get_density(
        ...
        if warp_field is not None:
            if self.use_hash_se3field:
                sample_inds = ray_samples.sample_inds
                warped_positions, warped_directions = warp_field(positions, directions, sample_inds, window_alpha)
            else:
                assert "warp" in code_dict
                warped_positions, warped_directions = warp_field(positions, directions, code_dict["warp"], window_alpha)
