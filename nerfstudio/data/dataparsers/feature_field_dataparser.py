...
@dataclass
class FeatureNerfstudioDataParserConfig(DataParserConfig):
...
class Nerfstudio(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in frames:
            ...
            if "feature_file_path" in frame:
                feature_filepath = Path(frame["feature_file_path"])
                feature_fname = self._get_fname(
                    feature_filepath, data_dir, downsample_folder_prefix="features_"
                )
                feature_filenames.append(feature_fname)
        ...
        """
        assert len(feature_filenames) == 0 or (
            len(feature_filenames) == len(image_filenames)
        ), """
        Different number of image and feature filenames.
        You should check that feature_file_path is specified for every frame (or zero frames) in transforms.json.
        """
        ...
        feature_filenames = (
            [feature_filenames[i] for i in indices]
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            metadata={
                ...
                "feature_filenames": (
                    feature_filenames if len(feature_filenames) > 0 else None
                ),
