...
class VanillaDataManagerConfig(DataManagerConfig):
    ...
    _target: Type = field(default_factory=lambda: DNeRFDataManagerFast)
    """Target class to instantiate."""
...
class DNeRFDataManagerFast(DataManager, Generic[TDataset]):
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        orig_class: Type[DNeRFDataManagerFast] = get_orig_class(self, default=None)  # type: ignore
        if type(self) is DNeRFDataManagerFast and orig_class is None:
            return default
        if orig_class is not None and get_origin(orig_class) is DNeRFDataManagerFast:
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            if get_origin(base) is DNeRFDataManagerFast:
    ...
    def next_train(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        for k, v in batch.items():
            if k != "image_idx" and v is not None:
                last_dim = v.shape[-1]
                batch[k] = v.reshape(-1, last_dim).to(self.device)
    ...
    def next_eval(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        coords = self.eval_ray_generator.image_coords[y, x]

        ray_bundle = self.eval_ray_generator.cameras.generate_rays(
