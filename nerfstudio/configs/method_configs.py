...
descriptions = {
    ...
    "feature-depth-nerfacto": "Feature Nerfacto with depth supervision.",
    ...
    "kplanes-dynamic": "Dynamic Kplanes model",
}
...
method_configs["feature-depth-nerfacto"] = TrainerConfig(
    method_name="feature-depth-nerfacto",
    steps_per_eval_batch=500,
    steps_per_save=2000,
    max_num_iterations=30000,
    mixed_precision=True,
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
            _target=VanillaDataManager[FeatureDepthDataset],
            pixel_sampler=PairPixelSamplerConfig(),
            dataparser=FeatureNerfstudioDataParserConfig(),
            train_num_rays_per_batch=4096,
            eval_num_rays_per_batch=4096,
        ),
        model=FeatureDepthNerfactoModelConfig(
            eval_num_rays_per_chunk=1 << 15,
            camera_optimizer=CameraOptimizerConfig(mode="SO3xR3"),
        ),
    ),
